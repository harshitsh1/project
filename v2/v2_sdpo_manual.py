...
class TrueSDPOConfig:
    ...
    output_dir     : str   = r"D:\deep_learning\outputs\v2_sdpo_manual"
...
def plot_results(output_dir: str):
    ...
    if not os.path.exists(log_path):
        print("[Plot] No training_log.json found, skipping.")
        return
    ...
    steps   = [h.get("step", 0) for h in log_history]
    losses  = [h.get("loss") for h in log_history]
    rewards = [h.get("reward") for h in log_history]
    
    # filter out None values for plotting
    valid_loss   = [(s, l) for s, l in zip(steps, losses) if l is not None]
    valid_reward = [(s, r) for s, r in zip(steps, rewards) if r is not None]

    if not valid_loss and not valid_reward:
        return

    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    fig.suptitle('SDPO v2 (Manual) Training Results', fontsize=14, fontweight='bold')

    if valid_loss:
        sx, sy = zip(*valid_loss)
        axes[0].plot(sx, sy, marker='.', color='#FF9800', linewidth=1.5, alpha=0.8)
        axes[0].set_title('Training Loss', fontweight='bold')
        axes[0].set_xlabel('Steps')
        axes[0].grid(True, linestyle='--', alpha=0.4)

    if valid_reward:
        sx, sy = zip(*valid_reward)
        axes[1].plot(sx, sy, marker='.', color='#4CAF50', linewidth=1.5, alpha=0.8)
        axes[1].set_title('Batch Reward', fontweight='bold')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["smoke", "small", "large", "plot"],
                        default="smoke",
                        help="smoke=5 steps, small=20 samples, large=500 samples, plot=generate graph")
