"""
SDPO implementation with train/test splits and evaluation metrics.
"""
...
class TrueSDPOConfig:
    ...
    output_dir     : str   = r"D:\deep_learning\outputs\v3_sdpo_eval"
    ...
    eval_steps          : int = 40   # was 10 -> half as many evals
...
# --- Environment Factory ---
def make_environment(prefer_semantic: bool = True):
    if prefer_semantic:
        try:
...
def evaluate(
    ...
    """
    Evaluates ONLY on verifiable_test using SimpleEnvironment.

    Why SimpleEnvironment (not SemanticEnvironment) here:
      exact_match needs a binary 0/1 signal, not a continuous score.
      SimpleEnvironment gives that reliably on fallback prompts.
      SemanticEnvironment is used during training for richer feedback.
    ...
    for q in verifiable_test:
        ...
        reward, feedback = simple_env.evaluate(q, response)
...
def train_sdpo(cfg: TrueSDPOConfig):
    ...
    simple_env  = SimpleEnvironment()
    train_env   = make_environment(prefer_semantic=True)  # richer feedback during training
    ...
    baseline        = evaluate(model, tokenizer, verifiable_test, simple_env,
    ...
    # Final eval
    print("\n[Eval] Final:")
    final = evaluate(model, tokenizer, verifiable_test, simple_env,
...
def plot_results(output_dir: str):
    ...
    train_path = os.path.join(output_dir, "train_log.json")
    if not os.path.exists(eval_path):
        print("[Plot] No eval_log.json found, skipping.")
        return
    ...
    fig.suptitle('SDPO v3 Training Results', fontsize=16, fontweight='bold')
    ...
    def _plot_eval(ax, y, title, color, ylabel=""):
        ...
        if len(y) >= 2:
            d = y[-1] - y[0]
            ax.annotate(f'D={d:+.3f}', xy=(steps[-1], y[-1]),
                        fontsize=9, color='green' if d >= 0 else 'red',
                        fontweight='bold', xytext=(-55, 12),
                        textcoords='offset points')

    def _plot_train(ax, xs, ys, title, color):
        valid = [(s, v) for s, v in zip(xs, ys) if v is not None]
        if valid:
            sx, sy = zip(*valid)
            ax.plot(sx, sy, marker='.', color=color, linewidth=1.5,
    ...
    _plot_eval(axes[0, 1], exact_match, 'Exact Match Rate',  '#4CAF50', 'Rate')
    axes[0, 1].set_ylim(-0.05, 1.05)

    ax = axes[0, 2]
    ax.plot(steps, refusal_rate, marker='^', color='#F44336', linewidth=2,
            markersize=5, label='Refusal')
    ax.set_title('Refusal Rate', fontweight='bold')
    ax.set_xlabel('Steps'); ax.set_ylim(-0.05, 1.05)
    ax.grid(True, linestyle='--', alpha=0.4)
    ax.fill_between(steps, refusal_rate, alpha=0.1, color='#F44336')

    _plot_train(axes[1, 0], t_steps, t_loss, 'Training Loss',       '#FF9800')
    _plot_train(axes[1, 1], t_steps, t_rew,  'Train Batch Reward',  '#3F51B5')
    _plot_eval(axes[1, 2],  avg_len,         'Avg Response Length',  '#9C27B0', 'Words')
...
def smoke_test():
    ...
    cfg.logging_steps = 1
    tokenizer, model = load_model(cfg)
    train_env = make_environment(prefer_semantic=True)
    simple_env = SimpleEnvironment()

    print("\n  Baseline eval (verifiable only):")
    baseline = evaluate(model, tokenizer, vt, simple_env, cfg, step=0)
    ...
    print("\n  Post eval:")
    post = evaluate(model, tokenizer, vt, simple_env, cfg,
...
if __name__ == "__main__":
    ...
    p.add_argument("--mode", choices=["smoke","small","large","plot"],
                   default="smoke")
    args = p.parse_args()
    if args.mode == "smoke":
        smoke_test()
